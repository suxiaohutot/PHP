<?php
class upload {
    private $allow_types = array('image/jpeg', 'image/peg', 'image/png', 'image/x-png', 'image/gif', 'image/jpg');
    //允许上传的文件类型数组
    private $max_size = 1048576;//允许上传的文件最大尺寸为1M
    private $upload_path = './';//上传的文件保存在服务器中的目录位置
    //上传文件时产生的错误
    private $error = '';
    /**
     * 构造方法
...
//         echo "<p class='box'>upload对象被销毁的时间为：".date("H:i:s").'</p>';
        echo "<p class='box'>上传成功 上传时间为：".date("H:i:s").'</p>'."<br><br>"."<a href='dengluhtml.php'>返回登录</a>";
    }
}